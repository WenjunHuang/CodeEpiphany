...
object problemsets {
  def fetchAndUpdateProblemSets(project: Project): IO[Unit] = {
    ...
    showConsole(project) *>
      ...
      (AtCoderApi.getAllProblemDifficulty, AtCoderApi.getAllProblems, AtCoderApi.getAllContests).parTupled.flatMap {
        ...
        case Right((problems, contests)) =>
          showConsole(project) *>
            console.info(project, AtCoder, PluginBundle.message("atcoder.problemsets.success", problems, contests))
        case Left(e) =>
          showConsole(project) *>
            console.error(project, AtCoder, PluginBundle.message("atcoder.problemsets.error", e.getMessage))
