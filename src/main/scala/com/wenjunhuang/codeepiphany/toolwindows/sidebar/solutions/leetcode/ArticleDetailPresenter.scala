...
class ArticleDetailPresenter(private val myProject:Project,private val myCodeDojo:CodeDojo.LeetCodeCN.type | CodeDojo.LeetCode.type) extends Disposable{
  ...
  private def createQueryPipeline(): Unit = {
    Stream
      .eval((Queue.unbounded[IO, Option[LeetCodeQuestionSolutionArticle]], SignallingRef.of[IO, Boolean](false)).parTupled)
          ...
          .flatMap { _ =>
            Stream
              ...
              .evalTap { (signal, context) =>
                Stream.eval {
                  LeetCodeApi(myCodeDojo,myProject).getSolutionArticle(context.slug)
                    .flatMap{article=>
                      IO.delay{
                        article.content
