...
trait CodeForcesApi {
  ...
  def getProblemTags: IO[List[String]]
  def getChallengeData(
    problemsetName: Option[String],
    contestId: Long,
    index: String
  ): IO[Option[CodeForcesChallengeData]]
...
object CodeForcesApi extends CodeForcesApi with Http4sClientDsl[IO] {
  ...
  override def checkLogin(): IO[Boolean] = {
    HttpClientManager.getClient.use { client =>
      client
        .get(uri"https://codeforces.com/settings/general") { response =>
          response.body.compile.to(ByteVector).flatMap { content =>
            ...
            if Jsoup.parse(html).select("div.userbox").isEmpty then IO.pure(false)
            else IO.pure(true)
          }
        }
  ...
  override def getProblemTags: IO[List[String]] = HttpClientManager.getClient.use { client =>
  ...
  private def getSubmitAnswerResult(
    ...
    csrfToken: String
  ): IO[CodeForcesSubmissionResponse] = {
    HttpClientManager.getClient.use { client =>
      client
        .expect[String](
          Method
