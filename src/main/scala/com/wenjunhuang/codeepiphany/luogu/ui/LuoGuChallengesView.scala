...
class LuoGuChallengesView(private val myProject: Project) extends BaseChallengesView[LuoGuUI] {
  ...
  private val myLoginLogoutProvider = new LoginLogoutProvider {
    override def login(): Unit = {
      myIsLoggingIn = true
      (console.info(myProject, LuoGu, PluginBundle.message("console.loggingIn", CodeDojo.LuoGu.show)) *>
        AuthService
          ...
          .flatMap {
            case AskForLoginResult.Done =>
              initialize().map { bootstrap =>
                ...
                *> console.info(myProject, LuoGu, PluginBundle.message("console.loggedIn", CodeDojo.LuoGu.show))
            case _ =>
              console.info(myProject, LuoGu, PluginBundle.message("console.loginCancelled", CodeDojo.LuoGu.show))
          }
          .handleErrorWith { e =>
            myLogger.warn(e)("Failed to login") *>
              console.error(
                myProject,
                LuoGu,
                PluginBundle.message("console.loginFailed", CodeDojo.LuoGu.show, e.getMessage)
              )
          })
        .guarantee(IO.delay { myIsLoggingIn = false })
        .unsafeRunAsBackgroundProgressCancellable(
          myProject,
          PluginBundle.message("console.loggingIn", CodeDojo.LuoGu.show)
        )
    }
            ...
            .flatMap { cookies =>
              IO.delay {
                val file = new WebPreviewVirtualFile(
                  s"https://www.luogu.com.cn/user/${userInfo.uid}#practice",
                  CodeDojo.LuoGu.domain.toString,
                  cookies,
                  CodeDojo.LuoGu.show
                )
